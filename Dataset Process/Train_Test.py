...
aim_directory = '.'
tracing = open("resort_processing.txt",'w')
tracing.close()

for dir in os.listdir(aim_directory):
    try:
        count = 0
        ...
        for file in os.listdir(judge_yes_directory):
            os.remove(judge_yes_directory+'/'+file)
        for file in os.listdir(judge_no_directory):
            os.remove(judge_no_directory+'/'+file)
        for file in os.listdir(picture_directory):
            os.remove(picture_directory+'/'+file)

        for file in os.listdir(sample_directory):
            family = dir.split(" ")[0]
            if family in file:
                copyfile(sample_directory+"/"+file,judge_yes_directory + "/" + file)
            else:
                copyfile(sample_directory+"/"+file,judge_no_directory + "/" + file)

        for file in os.listdir(dir):
            if ".jpg" in file:
                copyfile(dir+"/"+file,picture_directory + "/" + file)

        need_retrain = True
        IMAGE_DIR = judge_directory
        OUTPUT_GRPAH = resort_directory + "/Graphs/" + dir + "_Graph.pb"
        OUTPUT_LABELS = resort_directory + "/Labels/" + dir + "_Labels.txt"
        ...
        while need_retrain:
            try:
                ...
                OUTPUT_GRPAH = resort_directory + "/Graphs/" + dir + "_Graph" + str(time_count) + ".pb"
                OUTPUT_LABELS = resort_directory + "/Labels/" + dir + "_Labels" + str(time_count) + ".txt"
                tf.app.run()
            except:
                print(str(time_count) + " times tensorflow running on " + dir)
        ...
        for image_path in os.listdir(dir):
            try:
                # Read in the image_data
                image_data = tf.gfile.FastGFile(dir+'/'+image_path, 'rb').read()
                with tf.Session() as sess:
                    # Feed the image_data as input to the graph and get first prediction
                    softmax_tensor = sess.graph.get_tensor_by_name('final_result:0')
        ...
        tracing.write(dir + " finished, " + str(count) + " kinds of flower resorted.")
        tracing.close()
    except:
...
#IMAGE_DIR = '/Users/langlin/work/Year2/Project/Rose_or_not'
#OUTPUT_GRPAH = '/Users/langlin/work/Year2/Project/Final_test/Rose_test/myGraph.pb'
#OUTPUT_LABELS = '/Users/langlin/work/Year2/Project/Final_test/Rose_test/mylabels.txt'
#BOTTLENECK_DIR = '/Users/langlin/work/Year2/Project/Final_test/Rose_test/testMyModel/bottlenecks'
#SUMMARIES_DIR = '/Users/langlin/work/Year2/Project/Final_test/Rose_test/tensorflowLog'
