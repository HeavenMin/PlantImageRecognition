...
"""
 AUTHOR : Lang
 PURPOSE : Do the first process of Purification
 USAGE : python3 "this_file_name" "deep_learning_used_dataset_directory" "dataset_need_to_be_purify_directory"
"""
...
MODEL_DIR = '/mnt/Deep_Learning/Final/inception'
...
OUTPUT_GRPAH = os.path.abspath('.') + '/First_Purification/First_Purification_Graph.pb'
OUTPUT_LABELS = os.path.abspath('.') + '/First_Purification/First_Purification_Labels.txt'
BOTTLENECK_DIR = os.path.abspath('.') + '/First_Purification/bottlenecks'
SUMMARIES_DIR = os.path.abspath('.') + '/First_Purification/tensorflowLog'  # save logs
...
purify_dir = sys.argv[2]
image_dirs = sorted(os.listdir(purify_dir))
...
for image_dir in image_dirs:
    image_paths = sorted(os.listdir(purify_dir + '/' + image_dir))
    for image_path in image_paths:
        if ".jpg" not in image_path:
            continue
        image_data = tf.gfile.FastGFile(purify_dir + '/' + image_dir + '/' + image_path, 'rb').read()
        with tf.Session() as sess:
            ...
            for node_id in top_k:
                ...
                print(score)
                break
            if human_string == "no flower":
                shutil.move(purify_dir + '/' + image_dir + '/' + image_path, os.path.abspath('.')+'/First_Purification/Removed_Image/'+ image_path)
    count = count+1
    tracing = open("first_purification.txt",'a')
    tracing.write(str(count) + ". " + image_dir + " finished.\n")
    tracing.close()
